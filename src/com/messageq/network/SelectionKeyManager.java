...
public class SelectionKeyManager {
	...
	public boolean handleAllNodes(NIOServerHandler nioserverhandler,
			PtoPPacket p) {
		...
		ml.log("handle all nodes with the message " + new String(p.getArgs()));
		return true;
	}

	public void addnode(SelectionKey key) {
		synchronized (nodes) {
			String hostname = ((SocketChannel) key.channel()).socket()
	...
	public SelectionKey getOneNode(String hostname) {
		...
		ml.warn("you try to find a node named :" + hostname
				+ ",but nodes is empty or we don't have this node");
		return null;
	}
